...
#define OS_KERNEL_DIAG_ERROR_NONE      (0U)           /*                                       */
...
#define OS_KERNEL_DIAG_PRIORITY_ERROR  (0x1<<4U)      /* 优先级错误                            */


/* 内核变量结构定义，记录了内核运行时的各种数据 */
struct KernelVariableDef
{
    TBase32          SchedLocks;                  /* 决定是否允许线程调度                  */
...
extern void OsKernelAddObject(TObject* pObject, TChar* pName, TObjectType type);
