...
static void DispatchExpiredTimer(TTimer* pTimer)
{
    ...
    /*
     * 将定时器放入内核定时器期满列表，最后由定时器守护线程处理。紧急的定时器优先处理。
     */
...
void OsTimerTickUpdate(void)
{
    ...
    spoke = (TBase32)(OsKernelVariable.Jiffies % TCLC_TIMER_WHEEL_SIZE);
    ...
    {
        pNext = pNode->Next;
        pTimer = OS_CONTAINER_OF(pNode, TTimer, LinkNode);
    ...
    {
        OsThreadResume(OsKernelVariable.TimerDaemon);
    }
}
 ...
 *  说明                                                                                         *
 *************************************************************************************************/
TState TclCreateTimer(TTimer* pTimer, TChar* pName, TBitMask property, TTimeTick ticks,
                      TTimerRoutine pRoutine, TArgument data, TBase32 priority, TError* pError)
{
    TState state = eError;
    ...
    {
        /* 将定时器加入到内核对象队列中 */
        OsKernelAddObject(&(pTimer->Object), pName, OsObjTypeTimer);
 ...
 *  说明                                                                                         *
 *************************************************************************************************/
TState TclConfigTimer(TTimer* pTimer, TTimeTick ticks, TBase32 priority, TError* pError)
{
    TState state = eError;
    TError error = OS_TIMER_ERR_UNREADY;
    TBase32 imask;
...
static void TimerDaemonEntry(TArgument argument)
{
    ...
    while(eTrue)
    {
        ...
        {
            /* 从期满队列中取得一个定时器 */
            pTimer = OS_CONTAINER_OF(TimerExpiredList, TTimer, ExpiredNode);
