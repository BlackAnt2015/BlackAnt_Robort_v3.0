...
/* 中断向量描述符属性 */
#define IRQ_VECTOR_PROP_NONE   (TBitMask)(0x0)
#define IRQ_VECTOR_PROP_READY  (TBitMask)(0x1<<0)
#define IRQ_VECTOR_PROP_LOCKED (TBitMask)(0x1<<1)
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
void OsIrqEnterISR(TBase32 irqn)
{
    TBase32      imask;
    TIrqVector* pVector;
    TISR        pISR;
            ...
            (pVector->Property & IRQ_VECTOR_PROP_READY))
    {
        ...
        if (pVector->ISR != (TISR)0)
        {
             ...
             * 注意此时DAEMON可能处于OS_THREAD_STATUS_READY状态
             */
#if (TCLC_IRQ_DAEMON_ENABLE)
            if (retv & OS_IRQ_DAEMON)
            {
                OsThreadResume(OsKernelVariable.IrqDaemon);
            }
#endif
        }
        pVector->Property &= (~IRQ_VECTOR_PROP_LOCKED);
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclSetIrqVector(TBase32 irqn, TISR pISR, TArgument data, TError* pError)
{
    TState state = eError;
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclCleanIrqVector(TBase32 irqn, TError* pError)
{
    TState state = eError;
    TError error = OS_IRQ_ERR_FAULT;
    TBase32 imask;
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclPostIRQ(TIrq* pIRQ, TIrqEntry pEntry, TArgument data, TBase32 priority,
                  TError* pError)
{
    TState state = eError;
    TError error = OS_IRQ_ERR_FAULT;
...
static void IrqDaemonEntry(TArgument argument)
{
    ...
    while(eTrue)
    {
        ...
        {
            pIRQ   = OS_CONTAINER_OF(IrqReqList.Handle, TIrq, LinkNode);
            pEntry = pIRQ->Entry;
            data   = pIRQ->Argument;
            OsObjListRemoveNode(pIRQ->LinkNode.Handle, &(pIRQ->LinkNode));
            memset(pIRQ, 0U, sizeof(TIrq));
...
void OsIrqModuleInit(void)
{
    ...
    OsThreadCreate(&IrqDaemonThread,
                   ...
                   (TBase32)TCLC_IRQ_DAEMON_PRIORITY,
