 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
static TState ReceiveFlags(TFlags* pFlags, TBitMask* pPattern, TBitMask option, TError* pError)
{
    TState state = eError;
...
static TState SendFlags(TFlags* pFlags, TBitMask pattern, TBool* pHiRP, TError* pError)
{
    ...
    if (mask != pFlags->Value)
    {
        ...
        /* 事件标记是否有线程在等待事件的发生 */
        if (pFlags->Queue.Handle != (TLinkNode*)0)
        {
            /* 开始遍历事件组阻塞队列 */
            pHead = pFlags->Queue.Handle;
            pTail = pFlags->Queue.Handle->Prev;
            do
            {
                pCurrent = pHead;
                pHead = pHead->Next;
                ...
                /* 获得等待事件标记的线程和相关的事件节点 */
                pContext = OS_CONTAINER_OF(pCurrent, TIpcContext, LinkNode);
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclReceiveFlags(TFlags* pFlags, TBitMask* pPattern, TBitMask option, TTimeTick timeo,
                       TError* pError)
{
    TState state = eError;
                ...
                (OsKernelVariable.SchedLocks == 0U))
        {
            ...
            if (state == eError)
            {
                if (option & OS_IPC_OPT_WAIT)
                {
                    ...
                    OsIpcBlockThread(&context, (void*)pFlags, &(pFlags->Queue),
                                     (TBase32)pPattern, sizeof(TBase32),
                                     option | OS_IPC_OPT_FLAGS, timeo,
                                     &state, &error, &HiRP);
                    if (HiRP == eTrue)
                    {
                        OsThreadSchedule();
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclCreateFlags(TFlags* pFlags, TChar* pName, TBitMask property, TError* pError)
{
    TState state = eError;
    ...
    {
        /* 初始化事件标记对象信息 */
        OsKernelAddObject(&(pFlags->Object), pName, OsObjTypeFlag);

        /* 初始化事件标记基本信息 */
        pFlags->Property =(property | OS_IPC_PROP_READY);
        pFlags->Value    = 0U;

        pFlags->Queue.Handle   = (TLinkNode*)0;
...
TState TclDeleteFlags(TFlags* pFlags, TError* pError)
{
    ...
    if (pFlags->Property & OS_IPC_PROP_READY)
    {
        ...
        OsIpcUnblockAll(&(pFlags->Queue), eError, OS_IPC_ERR_DELETE, (void**)0, &HiRP);
...
TState TclResetFlags(TFlags* pFlags, TError* pError)
{
    ...
    if (pFlags->Property & OS_IPC_PROP_READY)
    {
        ...
        OsIpcUnblockAll(&(pFlags->Queue), eError, OS_IPC_ERR_RESET, (void**)0, &HiRP);
...
TState TclFlushFlags(TFlags* pFlags, TError* pError)
{
    ...
    if (pFlags->Property & OS_IPC_PROP_READY)
    {
        ...
        OsIpcUnblockAll(&(pFlags->Queue), eError, OS_IPC_ERR_FLUSH, (void**)0, &HiRP);
