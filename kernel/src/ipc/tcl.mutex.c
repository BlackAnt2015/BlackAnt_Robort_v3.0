...
static TState RemoveLock(TThread* pThread, TMutex* pMutex, TBool* pHiRP, TError* pError)
{
    TState state = eSuccess;
    TError error = OS_IPC_ERR_NONE;
    TBase32 priority = TCLC_LOWEST_PRIORITY;
    ...
    if (!(pThread->Property & OS_THREAD_PROP_PRIORITY_FIXED))
    {
        ...
        else
        {
            ...
            {
                /* 准备恢复线程优先级 */
                priority = *((TBase32*)(pThread->LockList->Data));
                nflag = eTrue;
            }
        }
...
static TState FreeMutex(TMutex* pMutex, TBool* pHiRP, TError* pError)
{
    ...
    if (pMutex->Owner == OsKernelVariable.CurrentThread)
    {
        ...
        if (pMutex->Nest == 0U)
        {
            ...
            if (pMutex->Queue.Handle != (TLinkNode*)0)
            {
                pContext = OS_CONTAINER_OF(pMutex->Queue.Handle, TIpcContext, LinkNode);
 ...
 * ●如果失败并且是阻塞方式访问互斥量，则将线程阻塞在互斥量的阻塞队列中，然后调度。
 */
TState TclLockMutex(TMutex* pMutex, TBitMask option, TTimeTick timeo, TError* pError)
{
    TState state = eError;
    TError error = OS_IPC_ERR_UNREADY;
    TIpcContext context;
        ...
        if (OsKernelVariable.SchedLocks == 0U)
        {
            if (state == eError)
            {
                if (option & OS_IPC_OPT_WAIT)
                {
                     ...
                     */
                    OsIpcBlockThread(&context, (void*)pMutex, &(pMutex->Queue), 0U, 0U,
                                     (option | OS_IPC_OPT_MUTEX), timeo,
                                     &state, &error, &HiRP);
                    if (HiRP == eTrue)
                    {
                        OsThreadSchedule();
 ...
 *  说明：                                                                                       *
 *************************************************************************************************/
TState TclCreateMutex(TMutex* pMutex, TChar* pName, TBitMask property, TBase32 priority,
                      TError* pError)
{
    TState state = eError;
    ...
    {
        /* 初始化互斥量对象信息 */
        OsKernelAddObject(&(pMutex->Object), pName, OsObjTypeMutex);
        ...
        pMutex->Queue.Handle   = (TLinkNode*)0;
...
TState TclDeleteMutex(TMutex* pMutex, TError* pError)
{
    ...
    if (pMutex->Property & OS_IPC_PROP_READY)
    {
        ...
        if (pMutex->Owner != (TThread*)0)
        {
             ...
             */
            OsIpcUnblockAll(&(pMutex->Queue), eError, OS_IPC_ERR_DELETE, (void**)0, &HiRP);
        }
    ...
    if (pMutex->Property & OS_IPC_PROP_READY)
    {
        ...
        if (pMutex->Owner != (TThread*)0)
        {
            ...
            /* 将阻塞队列上的所有等待线程都释放，所有线程的等待结果都是OS_IPC_ERR_RESET，
               而且这些线程的优先级一定不高于互斥量所有者的优先级 */
            OsIpcUnblockAll(&(pMutex->Queue), eError, OS_IPC_ERR_RESET, (void**)0, &HiRP);
...
TState TclFlushMutex(TMutex* pMutex, TError* pError)
{
    ...
    if (pMutex->Property & OS_IPC_PROP_READY)
    {
        ...
        OsIpcUnblockAll(&(pMutex->Queue), eError, OS_IPC_ERR_FLUSH, (void**)0, &HiRP);
